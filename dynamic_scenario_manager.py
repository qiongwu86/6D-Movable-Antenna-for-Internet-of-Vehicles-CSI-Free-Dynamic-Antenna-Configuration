...
class DynamicScenarioManager:
    ...
    def _allocate_antennas_with_optimization(self):
        ...
        for candidate in grid_antenna_candidates:
            ...
            # 计算该天线能覆盖的所有网格
            covered_grids = self._find_covered_grids(candidate['position'], candidate['normal'])
    ...
    def _find_covered_grids(self, antenna_pos: np.ndarray, antenna_normal: np.ndarray, 
                           coverage_radius: float = 200.0) -> Set[int]:
        """找到天线能覆盖的网格（简化覆盖模型）"""
        covered_grids = set()
        ...
        for grid_id, grid_info in self.grid_user_info.items():
            ...
            grid_center = grid_info.center_position
            
            # 简单的距离和方向判断
            distance = np.linalg.norm(grid_center - antenna_pos)
            if distance > coverage_radius:
                continue
            
            # 检查是否在天线辐射方向内（简化为半球）
            to_grid = grid_center - antenna_pos
            to_grid_normalized = to_grid / (np.linalg.norm(to_grid) + 1e-10)
            
            # 如果方向角度小于90度，认为可以覆盖
            dot_product = np.dot(antenna_normal, to_grid_normalized)
            if dot_product > 0:  # 朝向正确
    ...
    def _calculate_total_user_rate(self) -> float:
        ...
        user_rates = self._temp_env._calculate_theoretical_rates_vectorized(H)
